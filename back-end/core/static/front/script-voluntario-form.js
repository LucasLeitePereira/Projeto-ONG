    ...
    dataNascInput.addEventListener("change", function () {
      ...
      if (idadeReal < 18) {
        this.setCustomValidity(
          "Você deve ter pelo menos 18 anos para se voluntariar.",
        );
        showError(
          this,
          "Você deve ter pelo menos 18 anos para se voluntariar.",
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (isValid) {
      ...
      const dados = {
        ...
        estado:
          document.getElementById("enderecoestado_voluntario").value || "",
        cpfupload: true,
        fotoupload: true,
        termoupload: true,
      };

      const oabField = document.getElementById("oab_voluntarioadvogado");
      if (oabField && oabField.value.trim()) {
        dados.oab = oabField.value.trim();
      }

      const cursoBacharelField = document.getElementById(
        "cursoformacao_voluntariobacharel",
      );
      if (cursoBacharelField && cursoBacharelField.value.trim()) {
        dados.cursoBacharel = cursoBacharelField.value.trim();
      }

      const cursoEstagiarioField = document.getElementById(
        "cursonome_voluntarioestagiario",
      );
      const periodoEstagiarioField = document.getElementById(
        "cursoperido_voluntarioestagiario",
      );
      if (
        cursoEstagiarioField &&
        periodoEstagiarioField &&
        cursoEstagiarioField.value.trim() &&
        periodoEstagiarioField.value.trim()
      ) {
        dados.curso = cursoEstagiarioField.value.trim();
        dados.periodo = periodoEstagiarioField.value.trim();
      }
      ...
      fetch(rota, {
        ...
        .catch((error) => {
          ...
          alert(
            "Erro ao enviar cadastro: " + error.message + ". Tente novamente.",
          );
